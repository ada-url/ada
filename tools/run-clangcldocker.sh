...
ALL_ADA_FILES=$(cd $SCRIPTPATH/.. && git ls-tree --full-tree --name-only -r HEAD | grep -e ".*\.\(c\|h\|cc\|cpp\|hh\)\$")
tuser=$(echo $USER | tr -dc 'a-z')

container_name=${CONTAINER_NAME:-"ada-clang-format-for-$tuser"}
echo $container_name
...
docker run --rm $DOCKER_ARGS -h $container_name -v $MAINSOURCE:$MAINSOURCE:Z  -w $MAINSOURCE $container_name sh --style=file --verbose -i $ALL_ADA_FILES $COMMAND
