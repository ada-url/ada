...
if clang-format-22 --version  2>/dev/null | grep -qF 'version 22.'; then
  cd $MAINSOURCE; clang-format-22 --style=file --verbose -i "$@" $ALL_ADA_FILES
  exit 0
elif clang-format --version  2>/dev/null | grep -qF 'version 22.'; then
...
docker pull xianpengshen/clang-tools:22

docker run --rm $DOCKER_ARGS -v "$MAINSOURCE":"$MAINSOURCE":Z  -w "$MAINSOURCE" -u "$(id -u $USER):$(id -g $USER)" xianpengshen/clang-tools:22 clang-format --style=file --verbose -i "$@" $ALL_ADA_FILES
