...
int piped_file(Callable&& adaparse_print, const cxxopts::ParseResult result,
  ...
  while ((
      tr = read(input_fd, cachebuffer.get() + offset, cache_length - offset))) {
    howmany += tr;
    blocks++;
  ...
  if (result.count("benchmark")) {
    ...
    fmt::print(
        "read {} bytes in {} ns using {} lines, total_bytes is {} used {} "
        "loads\n",
...
int main(int argc, char** argv) {
  ...
  if (output_filename == "output.txt") {
    out.print(
        "By default, outputting to a file is disabled. Please enable -o flag "
        "(optionally with a path to activate it).");
  }
