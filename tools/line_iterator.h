...
struct line_iterator {
  ...
  inline std::string_view grab_line() noexcept {
    std::string_view thisline = all_text.substr(0, next_end_of_line); //advance to next EOL
    all_text.remove_prefix(next_end_of_line + 1); //remove anything prior to said EOL
    //this is a view!!! 
    return thisline;
  }
  inline size_t tail() const noexcept { return all_text.size(); } //return whats remaining
};
