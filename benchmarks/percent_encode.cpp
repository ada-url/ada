    ...
    for (size_t i = 0; i < N; i++) {
      ...
      for (std::string &url_string: examples) {
        benchmark::DoNotOptimize(
                              ada::unicode::percent_encode(url_string, ada::character_sets::FRAGMENT_PERCENT_ENCODE));
      }
...
static void SpecialQuery(benchmark::State &state) {
  ...
  if (collector.has_events()) {
    ...
    for (size_t i = 0; i < N; i++) {
      ...
      collector.start();
      for (std::string &url_string: examples) {
        benchmark::DoNotOptimize(ada::unicode::percent_encode(url_string,
...
static void UserInfo(benchmark::State &state) {
  ...
  if (collector.has_events()) {
    ...
    for (size_t i = 0; i < N; i++) {
      ...
      for (std::string &url_string: examples) {
        benchmark::DoNotOptimize(
                              ada::unicode::percent_encode(url_string, ada::character_sets::USERINFO_PERCENT_ENCODE));
      }
      std::atomic_thread_fence(std::memory_order_release);
      event_count allocate_count = collector.end();
      aggregate << allocate_count;
...
static void C0Control(benchmark::State &state) {
  for (auto _: state) {
    for (std::string &url_string: examples) {
      benchmark::DoNotOptimize(
                            ada::unicode::percent_encode(url_string, ada::character_sets::C0_CONTROL_PERCENT_ENCODE));
    }
  }
  if (collector.has_events()) {
    event_aggregate aggregate{};
