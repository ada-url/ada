    ...
    while ((i < lhs.first.size() || low_surrogate1 != 0) &&
      ...
      if (low_surrogate1 != 0) {
        ...
        if (c1 <= 0x7F) {
          ...
          // Truncated or invalid UTF-8: treat byte as its raw value and advance
          codePoint1 = c1;
          i++;
        }
      }
